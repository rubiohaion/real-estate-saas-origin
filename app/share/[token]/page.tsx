...
type ReportData = {
  ...
  valuationSummary?: { intendedUse?: string; valuationCommentary?: string; limitingConditions?: string; estimatedValue?: number; pricePerSqft?: number; confidence?: string; valuationMethod?: string; aiSource?: string; aiGeneratedAt?: string; adjustedValue?: number | null; finalValue?: number | null; valueSource?: string };
...
function money(v?: number | null) { if (v === null || v === undefined || Number.isNaN(v)) return "—"; return `$${Math.round(v).toLocaleString()}`; }
...
export default function SharedReportPage() {
  ...
  const finalValue = vs.finalValue ?? vs.adjustedValue ?? vs.estimatedValue ?? null;

  const createdDate = useMemo(() => { if (!report?.created_at) return "—"; try { return new Date(report.created_at).toLocaleDateString(); } catch { return "—"; } }, [report?.created_at]);

  if (loading) return <div style={{ padding: 32 }}>Loading…</div>;
  if (!report || err) return <div style={{ padding: 32, fontFamily: "Arial, Helvetica, sans-serif" }}><div style={{ fontWeight: 800, fontSize: 18 }}>Link unavailable</div><div style={{ marginTop: 8, color: "#b91c1c" }}>{err ?? "—"}</div></div>;
    ...
    <div style={{ background: "#e5e7eb", minHeight: "100vh", padding: 28 }}>
      <div className="page" style={pageStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 14, marginBottom: 18 }}>
          <div><div style={{ fontSize: 26, fontWeight: 800 }}>Residential Appraisal Report</div><div style={{ marginTop: 6, fontSize: 12, color: "#6b7280" }}><div><strong>Report ID:</strong> {report.id}</div><div><strong>Created:</strong> {createdDate}</div><div><strong>Status:</strong> FINAL</div><div><strong>Shared Link:</strong> {text(token)}</div></div></div>
        ...
        <div style={sectionBox}><div style={sectionTitle}>Valuation Commentary</div><table style={tableStyle}><tbody><tr style={rowStyle}><td style={labelCell}>Intended Use</td><td style={valueCell}>{text(vs.intendedUse)}</td></tr><tr style={rowStyle}><td style={labelCell}>Price per Sqft</td><td style={valueCell}>{vs.pricePerSqft ? `$${vs.pricePerSqft}` : "—"}</td></tr></tbody></table><div style={{ marginTop: 12 }}><div style={blockLabel}>Valuation Commentary</div><div style={blockText}>{textBlock(vs.valuationCommentary)}</div></div><div style={{ marginTop: 12 }}><div style={blockLabel}>Limiting Conditions</div><div style={blockText}>{textBlock(vs.limitingConditions)}</div></div></div>
        ...
        <div style={{ ...sectionBox, border: "2px solid #111" }}><div style={sectionTitle}>Final Opinion of Value</div><table style={tableStyle}><tbody><tr style={rowStyle}><td style={labelCell}>Final Value Used in Report</td><td style={{ ...valueCell, fontSize: 18, fontWeight: 900 }}>{money(finalValue)}</td></tr><tr style={rowStyle}><td style={labelCell}>Value Source</td><td style={valueCell}>{text(vs.valueSource || (vs.adjustedValue ? "Appraiser Adjusted" : "Internal Valuation"))}</td></tr><tr style={rowStyle}><td style={labelCell}>Calculated Estimate</td><td style={valueCell}>{money(vs.estimatedValue)}</td></tr><tr style={rowStyle}><td style={labelCell}>Appraiser Adjusted Value</td><td style={valueCell}>{money(vs.adjustedValue)}</td></tr></tbody></table></div>
