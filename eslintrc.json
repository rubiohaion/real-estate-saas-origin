...
export default function ReportViewPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await supabase.from("reports").select("id,status,created_at,report_data,address,city,state,zip").eq("id", id).single();
      if (error || !data) { router.replace("/"); return; }
      if (!isFinalStatus((data as any).status)) { router.replace(`/reports/${id}`); return; }
      setReport(data as ReportRow); setLoading(false);
    })();
  }, [id, router]);
    ...
    <>
      ...
      <div style={{ background: "#e5e7eb", minHeight: "100vh", padding: 28 }}>
        <div className="page" style={pageStyle}>
          ...
          <div className="no-print" style={{ display: "flex", gap: 12, marginTop: 18, flexWrap: "wrap" }}><button onClick={() => window.print()} style={btn}>Print / Save PDF</button><button onClick={() => window.location.href = `/reports/${report.id}`} style={btn}>Back to Report</button><button onClick={() => window.location.href = "/"} style={btn}>Back to Dashboard</button></div>
        </div>
      </div>
    </>
  );
